...
<head>
    ...
    <title>UDM Study Plan Generator - Subjects by Course</title>
...
<body>
    <div class="container">
        ...
        <div class="main">
            <div class="main-header">
                <div>
                    <h1 class="main-title">Subjects by Program</h1>
                    <p class="sub-title">Manage subjects for each Program</p>
                </div>
        ...
        <div class="content-card course-section">
            ...
            <?php
            $cid = $course['id'];
            $subjects = $conn->query("SELECT * FROM subjects WHERE course_id = $cid ORDER BY year_level, semester, code");

            $grouped_subjects = [];
            while ($sub = $subjects->fetch_assoc()) {
                $year = $sub['year_level'];
                $sem = $sub['semester'];
                $grouped_subjects[$year][$sem][] = $sub;
            }
            ?>
    ...
    if ($semester == '1st') {
        ...
        echo "Unknown Semester"; // If the value is unexpected
    }
    ?>
</h4>
                    ...
                    <div class="table-container">
                        <table class="data-table">
                            ...
                            <?php foreach ($subs as $sub): ?>
                                <?php
                                $sub_id = $sub['id'];
                                $prereq_result = $conn->query("SELECT s.code FROM subject_prerequisites sp JOIN subjects s ON sp.prerequisite_id = s.id WHERE sp.subject_id = $sub_id");
                                $prereqs = [];
                                while ($pr = $prereq_result->fetch_assoc()) {
                                    $prereqs[] = $pr['code'];
                                }
                                ?>
                                <tr>
                                    ...
                                    <td><?= implode(', ', $prereqs) ?></td>
                                    <td class="actions">
                                        <a href="edit.php?id=<?= $sub_id ?>" class="btn btn-sm btn-edit"><i class="fas fa-edit"></i></a>
                                        <a href="delete.php?id=<?= $sub_id ?>" class="btn btn-sm btn-delete" onclick="return confirm('Are you sure you want to delete this subject?');"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
